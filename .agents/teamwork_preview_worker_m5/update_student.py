...
export default function StudentDashboard() {
    ...
    const handleSubmitProject = async (projectId: number) => {
        ...
        try {
            ...
            showToast("Project successfully submitted for government review!", "success");
            setProjectModalOpen(false);
            await fetchData();
        } catch (e: any) {
            console.error(e);
        ...
        <div className="min-h-screen flex flex-col bg-background pb-20">
            ...
            <header className="sticky top-0 z-40 bg-gradient-to-r from-primary to-primary-container shadow-sm border-b border-primary/20">
                <div className="max-w-[1200px] mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-primary-fixed text-on-primary-fixed flex items-center justify-center font-bold text-xs ring-2 ring-primary-fixed/30">
                            {userInitials}
                        </div>
                        <button 
                            type="button" 
                    ...
                    <div className="space-y-3">
                        ...
                        {!loading && filteredIssues.map(issue => (
                            <div key={issue.id} className="bg-surface-container-lowest rounded-2xl p-4 shadow-sm border border-outline-variant/30 hover:border-primary/50 transition-colors">
                                ...
                                <p className="text-xs text-on-surface-variant mb-3 line-clamp-2">{issue.challenge_summary || issue.description}</p>
            ...
            <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-2 bg-surface-container-lowest/95 backdrop-blur-md shadow-[0_-2px_10px_rgba(0,0,0,0.05)] border-t border-outline-variant/30">
                {[
                    ...
                    { label: 'Profile', icon: 'person', action: () => { showComingSoon("Student Profile Settings"); setActiveNav('Profile'); } }
                ].map(nav => (
                    <button 
                        key={nav.label} 
                        type="button" 
                            ...
                            <div className="flex items-center justify-between pt-3 border-t border-outline-variant/20">
                                {selectedProject.status !== 'submitted' && (
                                    <button 
                                        ...
                                        className="px-4 py-2 bg-secondary text-on-secondary rounded-xl text-xs font-bold active:scale-95 transition-all flex items-center gap-1 disabled:opacity-50"
                                    >
...
with open(r"frontend/src/features/student/StudentDashboard.tsx", "w", encoding="utf-8") as f:
    f.write(code)
print("Successfully written StudentDashboard.tsx")
